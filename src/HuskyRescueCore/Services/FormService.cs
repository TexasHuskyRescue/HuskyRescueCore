...
namespace HuskyRescueCore.Services
{
    ...
    public class FormService : IFormSerivce
    {
        ...
        public FormService(ISystemSettingService systemServices, ApplicationDbContext context, ILogger<FormService> logger)
        {
        ...
        public async Task<string> CreateAdoptionApplicationPdf(ApplicationAdoption app)
        {
            ...
            try
            {
                //if (appType.Equals("A"))
                //{
                var genPath = (await _systemServices.GetSettingAsync("AdoptionApplicationOutputPath")).Value;

                // get location of template
                var pdfTempFilePath = (await _systemServices.GetSettingAsync("AdoptionApplicationFormPath")).Value;
                //}
                //else
                //{
                ...
                using (var newPdfFileStream = new FileStream(pdfNewFilePath, FileMode.Create))
                {
                    ...
                    var pdfStamper = new PdfStamper(pdfReader, newPdfFileStream);
                    try
                    {
